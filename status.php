<?php
session_start();
require_once("include/set_status.php");
if (!is_logged_in()) {
...
if (is_admin()) { //Если пользователь админ то вывожу код для админа ?>
    ...
    <main id="js-page-content" role="main" class="page-content mt-3">
        ...
        <form action="include/set_status.php" method="POST">
            <div class="row">
                <div class="col-xl-6">
                            ...
                            <div class="panel-content">
                                <div class="row">
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="hidden" name="id"
                                                   value="<?php echo $return_user_id['id']; ?>">
                                            <select class="form-control" id="example-select" name="select_status">
                                                <?php
                                                foreach ($return_status as $status_value) {
                                                    if ($status_value['id'] == $return_user_id['status']) { ?>
                                                        <option value="<?= $status_value['id'] ?>"
                                                                selected="selected"><?= $status_value['name'] ?></option>
                                                    <?php } else { ?>
                                    ...
                                    <div class="col-md-12 mt-3 d-flex flex-row-reverse">
                                        <button class="btn btn-warning" name="set_status">Set Status</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-4">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" id="example-select" name="select_status">
                                                    <?php
                                                    foreach ($return_status as $status_value) { //Выводим список статуса из БД
                                                        if ($status_value['id'] == $user['status']) { ?>
                                                            <option value="<?= $status_value['id'] ?>"
                                                                    selected="selected"><?= $status_value['name'] ?></option>
                                                        <?php } else { ?>
                                                            <option value="<?= $status_value['id'] ?>"><?= $status_value['name'] ?></option>
