<?php
session_start();
require_once("include/load_media_handler.php");
if (!is_logged_in()) {
    redirect_to('../page_login.php');//Если пользлватель не авторизирован то перенаправляю на страницу с авторизацией
} else {
    $user = $_SESSION['user'];//Получаю авторизированого пользователя и записываю его в переменную
}
if (is_admin()) { //Если пользователь админ то вывожу код для админа ?>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary bg-primary-gradient">
        ...
        <button aria-controls="navbarColor02" aria-expanded="false" aria-label="Toggle navigation"
                class="navbar-toggler" data-target="#navbarColor02" data-toggle="collapse" type="button"><span
    ...
    <main id="js-page-content" role="main" class="page-content mt-3">
        ...
        <form action="include/load_media_handler.php" method="POST" enctype="multipart/form-data">
            <div class="row">
                <div class="col-xl-6">
                    <div id="panel-1" class="panel">
                        <div class="panel-container">
                            ...
                            <div class="panel-content">
                                <input type="hidden" name="id" value="<?php echo $return_user_id['id']; ?>">
                                <?php if ($return_user_id['image'] == "" || $return_user_id['image'] == NULL) { ?>
                                ...
                                <?php } else { ?>
                                    <div class="form-group">
                                        <img src="uploads/<?php echo $return_user_id['image']; ?>" alt=""
                                             class="img-responsive" width="200">
                                    </div>
                                <?php } ?>
                                ...
                                <div class="col-md-12 mt-3 d-flex flex-row-reverse">
                                    <button class="btn btn-warning" name="load_image">Загрузить</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    if (is_author($logged_user_id, $edit_user_id)) { //Проверка юзера, автор ли пользователь профиля или нет ?>
        <!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
